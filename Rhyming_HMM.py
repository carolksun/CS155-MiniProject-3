...
class HiddenMarkovModel:
    ...
    def generate_rhyme_pairs(self, M, obs_map = None, rhyming_dict = None):
        '''
        ...
        states1 = []
        states2 = []
        #choose a random rhyme
        rhyme1, rhyme2 = random.choice(rhyming_dict)
        
        #get emission index from obsmap
        index1 = obs_map[rhyme1]
        index2 = obs_map[rhyme2]
        
        #self.O[][emission index]
        probs_index1 = [row[index1] for row in self.O]
        probs_index2 = [row[index2] for row in self.O]
        
        #append to states pick state with highest probability
        state1 = probs_index1.index(max(probs_index1))
        state2 = probs_index2.index(max(probs_index2))
            
        #append emissions and state
        emission1.append(index1)
        emission2.append(index2)
        states1.append(state1)
        states2.append(state2)

        for i in range(M - 1):
            state1 = np.random.choice(list(range(self.L)), p = self.A[state1])
            states1.append(state1)
            element = np.random.choice(list(range(self.D)), p = self.O[state1])
            emission1.append(element)
            
            state2 = np.random.choice(list(range(self.L)), p = self.A[state2])
            states2.append(state2)
            element = np.random.choice(list(range(self.D)), p = self.O[state2])
            emission2.append(element)

        return emission1, states1, emission2, states2

    def sonnet(self, M, obs_map = None, rhyming_dict = None):
        rhyme1 = []
        rhyme2 = []
        sonnet = []
        
        for i in range(7):
            emission1, _, emission2, _ = self.generate_rhyme_pairs(M, obs_map, rhyming_dict)
            rhyme1.append(emission1)
            rhyme2.append(emission2)
            
        for i in range(0, 6, 2):
            sonnet.append(rhyme1[i])
            sonnet.append(rhyme1[i + 1])
            sonnet.append(rhyme2[i])
            sonnet.append(rhyme2[i + 1])
        
        sonnet.append(rhyme1[-1])
        sonnet.append(rhyme2[-1])
