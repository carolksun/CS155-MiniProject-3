...
def sample_sentence(hmm, obs_map, n_words=100, rhyming_dict = None):
    # Get reverse map.
    obs_map_r = obs_map_reverser(obs_map)

    # Sample and convert sentence.
    sonnet = hmm.sonnet(n_words, obs_map, rhyming_dict)
    
    sonnet_lines = ''
    
    for sentence in sonnet:
        sentence_mapped_list = [obs_map_r[sentence[i]] for i in range(len(sentence) - 1, -1, -1)]
        real_sentence = ' '.join(sentence_mapped_list).capitalize()
        sonnet_lines += real_sentence + '\n'
